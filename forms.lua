...
function TComponent:draw()
    ...
    else self.X = self.left self.Y = self.top
    end
    ...
    else gpu.fill(self.X, self.Y, self.W, self.H, " ")
    end
    self:paint()
...
function TButton:paint()
    gpu.set(self.X + (self.W - unicode.len(self.caption)) / 2, self.Y + (self.H - 1) / 2, self.caption)
end
...
function TLabel:paint()
    ...
    if self.autoSize then
        ...
        for line in value:gmatch("([^\n]+)") do
            self.H = self.H + 1
            if unicode.len(line) > self.W then self.W = unicode.len(line) end
        end
    ...
    for i = 0, self.H - 1 do
        ...
        if self.centered then gpu.set(self.X + (self.W - unicode.len(line)) / 2, self.Y + i, line)
        else
            if self.alignRight then gpu.set(self.X + self.W - unicode.len(line), self.Y + i, line)
            else gpu.set(self.X, self.Y + i, line)
            end
        end
    end
end
...
function TEdit:paint()
    if type(self.text) == "table" then
        for i = 1, self.H - 2 do gpu.set(self.X + 1, self.Y + i, unicode.sub(self.text[i] or "", 1, self.W - 2)) end
    else gpu.set(self.X + 1, self.Y + 1, unicode.sub(self.text, 1, self.W - 2))
    end
end

local function editText(text, left, top, W, H, validator)
    local running = true
    ...
    local function setCursor(nx, ny)
        ...
        if posX > unicode.len(text[posY]) + 1 then posX = unicode.len(text[posY]) + 1 end
        ...
        else term.setCursor(left + posX - scrollX - 1, top + posY - scrollY - 1)
        end
    end

    local function writeLine(n)
        gpu.set(left, top + n - scrollY - 1, padRight(unicode.sub(text[n] or "", scrollX + 1, scrollX + W), W))
    end
    ...
    local function insert(value)
        ...
        if not value or unicode.len(value) < 1 then return end
        text[posY] = unicode.sub(text[posY], 1, posX - 1) .. value .. unicode.sub(text[posY], posX)
        writeLine(posY)
        setCursor(posX + unicode.len(value))
    end

    local keys = {}
    keys[203] = function() -- Left
        if posX > 1 then setCursor(posX - 1)
        else if posY > 1 then posY = posY - 1 setCursor(unicode.len(text[posY]) + 1) end
        end
    end
    keys[205] = function() -- Right
        if posX <= unicode.len(text[posY]) then setCursor(posX + 1)
        else if posY < #text then setCursor(1, posY + 1) end
        end
    end
    keys[199] = function() setCursor(1) end -- Home
    keys[207] = function() setCursor(unicode.len(text[posY]) + 1) end -- End
    keys[211] = function() -- Del
        if posX <= unicode.len(text[posY]) then
            text[posY] = unicode.sub(text[posY], 1, posX - 1) .. unicode.sub(text[posY], posX + 1)
            writeLine(posY)
        else
            if posY < #text then
    ...
    keys[14] = function() -- BackSp
        if posX > 1 then
            text[posY] = unicode.sub(text[posY], 1, posX - 2) .. unicode.sub(text[posY], posX)
            writeLine(posY)
            setCursor(posX - 1)
        else
            if posY > 1 then
                posX, posY, text[posY - 1] = unicode.len(text[posY - 1]) + 1, posY - 1, text[posY - 1] .. text[posY]
                table.remove(text, posY + 1)
                writeText()
            end
        end
    ...
    local function onKeyDown(char, code)
        ...
        else if not isControl(char) then
    ...
    if type(text) == "table" then
        ...
        keys[28] = function() -- Enter
            local n = unicode.len(text[posY]:match("^%s*"))
            table.insert(text, posY + 1, string.rep(" ", n) .. unicode.sub(text[posY], posX))
            text[posY] = unicode.sub(text[posY], 1, posX - 1)
            posX, posY = n + 1, posY + 1
            writeText()
        end
    else
        posX = unicode.len(text) + 1
        text = { tostring(text) }
...
function TEdit:touch(x, y, btn, user)
    if btn == 0 then
        gpu.setBackground(self.color)
        gpu.setForeground(self.fontColor)
        if type(self.text) == "table" then editText(self.text, self.X + 1, self.Y + 1, self.W - 2, self.H - 2, self.validator)
        else self.text = editText(self.text, self.X + 1, self.Y + 1, self.W - 2, 1, self.validator)
        end
        self:draw()
...
function TList:paint()
    ...
    for i = 1, self.H - 2 * b do
        ...
        gpu.set(self.X + b, self.Y + i + b - 1, padRight(unicode.sub(self.lines[i + self.shift] or "", 1, self.W - 2 * b), self.W - 2 * b))
    ...
    for i = 1, #self.lines - 1 do
        for j = i + 1, #self.lines do
            if comp(self, i, j) then
                ...
                elseif self.index == j then self.index = i
                end
...
function forms.run(form)
    ...
    while work do
        ...
        if activeForm.mouseEv and adr == gpu.getScreen() then activeForm:mouseEv(ev, x, y, btn, user) end
